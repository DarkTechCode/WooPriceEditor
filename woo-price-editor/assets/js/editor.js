...
(function($) {
    ...
    const Config = {
        ...
        nonce: wpeData.nonce,
        ajaxUrl: wpeData.ajaxUrl,
        pageLength: parseInt(wpeData.pageLength, 10) || 50,
        defaultColumns: wpeData.defaultColumns || ['product', 'sku', 'regular_price', 'sale_price', 'stock_status', 'tax_status'],
        startCategory: wpeData.startCategory || 'all',
        debounceDelay: 300,
    ...
    const UI = {
         ...
         */
        showLoading() {
            this.$loading.attr('aria-hidden', 'false');
        },
         ...
         */
        hideLoading() {
            this.$loading.attr('aria-hidden', 'true');
        },
    ...
    const Filters = {
        currentFilters: {
            status: '',
            category: Config.startCategory !== 'all' ? Config.startCategory : '',
            tax_status: '',
            stock_status: '',
            search: ''
        },
            ...
            ApiClient.getCategories()
                .done((response) => {
                    if (response.success && response.data) {
                        ...
                        if (Config.startCategory !== 'all') {
                            $select.val(Config.startCategory);
                        }
                    }
                });
        },
        ...
        reset() {
            ...
            // Restore start category
            if (Config.startCategory !== 'all') {
                $('#wpe-category-filter').val(Config.startCategory);
                this.currentFilters.category = Config.startCategory;
            }
    ...
    const ColumnVisibility = {
        ...
        /**
         * Apply initial visibility from default columns and checkboxes
         */
        applyInitialVisibility() {
            $('.wpe-column-toggle input').each((i, checkbox) => {
                ...
                // Check if this column should be visible based on default columns
                const shouldBeVisible = Config.defaultColumns.includes(column);
                
                // Set checkbox state
                $checkbox.prop('checked', shouldBeVisible);
                
                // Apply visibility if not visible
                if (!shouldBeVisible) {
